...
const SeatSelectionPage = () => {
  ...
  const handleSeatClick = (seat) => {
    const isSoldOut = seat.bookings?.some(b => b.status === 'locked' || b.status === 'booked');
    if (!seat.is_active || seat.seat_type !== "seat" || isSoldOut) return;
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      // 1. Lock Seats in Backend via Thunk
      // This creates the 'LOCKED' records and the Razorpay Order
      await dispatch(confirmAndLockSeats({
        show_id: showId,
        seat_ids: selectedSeats.map(s => s.id)
      })).unwrap();

      // 2. Success: Navigate to the Payment Checkout Page
      // The PaymentPage will handle opening the Razorpay modal
      toast.success("Seats reserved! Proceed to payment.");
      navigate(`/booking/checkout/${showId}`);

    } catch (error) {
      // Handle backend error (e.g., "Seats no longer available")
  ...
  if (loading || !layout) return (
    <div className="min-h-screen bg-[#020617] flex items-center justify-center text-indigo-400 font-black uppercase tracking-widest">
      ...
      <div className="max-w-[1600px] mx-auto grid grid-cols-12 gap-8">
        ...
        <aside className="col-span-12 lg:col-span-3">
          ...
          <div className="bg-slate-900/40 p-6 rounded-[32px] border border-slate-800 backdrop-blur-md">
            <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4">Venue Legend</h2>
            <div className="space-y-3">
              <div className="flex items-center gap-3 text-xs font-bold text-slate-400">
                <div className="w-4 h-4 rounded-md border-2 border-slate-700 bg-slate-800/50"></div> Available
              </div>
              <div className="flex items-center gap-3 text-xs font-bold text-slate-400">
                <div className="w-4 h-4 rounded-md bg-indigo-500"></div> Selected
              </div>
              <div className="flex items-center gap-3 text-xs font-bold text-slate-400">
                <div className="w-4 h-4 rounded-md bg-slate-800 border-2 border-slate-700 opacity-20 relative flex items-center justify-center text-[8px]">✕</div> Sold Out
              </div>
            </div>
          </div>
        </aside>
          ...
          <div className="bg-slate-950 rounded-[56px] p-16 border border-slate-800 overflow-auto flex flex-col items-center shadow-2xl">
            ...
            <div className="grid relative" style={{ 
              ...
              {Object.entries(rowLabelsMap).map(([y, label]) => (
                <div key={y} className="flex items-center justify-center text-xs font-black text-slate-700 uppercase" style={{ gridRow: Number(y) + 1, gridColumn: 1 }}>
                  {label}
                </div>
              ))}

              {sectionedGraph.map((section) => (
                <div key={section.id} className="relative border-2 rounded-[28px] bg-slate-900/10 transition-all duration-500" style={{
                  gridColumnStart: section.minX + 2, gridColumnEnd: section.maxX + 3,
                  gridRowStart: section.minY + 1, gridRowEnd: section.maxY + 2,
                  borderColor: `${section.color}40`,
                }}>
                  <div className="absolute -top-3.5 left-6 px-3 bg-[#020617] text-[10px] font-black uppercase tracking-[0.2em] whitespace-nowrap z-10" style={{ color: section.color }}>
                    {section.name} • ₹{section.price}
                  </div>

                  {section.seats.map((seat) => {
                    const isSelected = selectedSeats.some(s => s.id === seat.id);
                    const bookingStatus = seat.bookings?.find(b => b.status === 'locked' || b.status === 'booked');
                    const isSoldOut = !!bookingStatus;
                    ...
                    return (
                      <button
                        ...
                        disabled={!seat.is_active || isSoldOut}
                        className={`absolute flex items-center justify-center transition-all duration-300 border-2 rounded-xl text-[10px] font-black ${isSelected ? 'scale-110 shadow-xl z-20' : 'hover:scale-110 z-10'} ${isSoldOut ? 'opacity-20 bg-slate-800 cursor-not-allowed' : 'cursor-pointer'}`}
                        style={{
                          width: `${CELL_SIZE}px`, height: `${CELL_SIZE}px`,
                          left: `${(seat.x_position - section.minX) * (CELL_SIZE + GAP_SIZE)}px`,
                          top: `${(seat.y_position - section.minY) * (CELL_SIZE + GAP_SIZE)}px`,
                          backgroundColor: isSelected ? section.color : isSoldOut ? '#1e293b' : 'transparent',
                          borderColor: isSoldOut ? '#334155' : isSelected ? '#fff' : section.color,
                          color: isSelected ? '#000' : isSoldOut ? '#475569' : section.color,
                        }}
                      >
                        {isSoldOut ? '✕' : `${seat.row_label}${seat.seat_number}`}
                      </button>
                    );
                  })}
                </div>
            ...
            <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[95%] max-w-5xl bg-slate-900/95 backdrop-blur-2xl border border-white/10 p-8 rounded-[40px] shadow-2xl flex items-center justify-between z-50 animate-in slide-in-from-bottom-10 duration-500">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-4">
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Tickets Selected</p>
