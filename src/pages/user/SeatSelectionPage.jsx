...
const SeatSelectionPage = () => {
  ...
  /**
   * Helper to determine seat availability based on the new 
   * hierarchical (Parent-Child) model.
   */
  const getSeatStatus = (seat) => {
    if (!seat.seat_bookings || seat.seat_bookings.length === 0) return null;

    // Look for a booking that is either fully BOOKED or still validly LOCKED
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      // confirmAndLockSeats now creates 1 Parent Booking and multiple SeatBooking entries
      await dispatch(
        confirmAndLockSeats({
          show_id: showId,
