...
import { updateEventLocation, fetchFullEventDetails } from "../../store/event/eventThunk";
...
export default function EditEventLocation() {
  ...
  useEffect(() => {
    if (event && event.shows && event.shows.length > 0) {
      const existingVenue = event.shows[0].venue;
      if (existingVenue) {
        setVenueName(existingVenue.name);
        setAddress(existingVenue.formatted_address);
        setLocation({ lat: existingVenue.latitude, lng: existingVenue.longitude });
      }
    }
  }, [event]);
  ...
  const submit = async () => {
    ...
    if (!venueName) return setFormError("Venue details are required.");
    ...
    try {
      await dispatch(updateEventLocation({ eventId, payload })).unwrap();
          ...
          <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <h1 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tighter">
                Edit Event <span className="text-indigo-600">Location</span>
              </h1>
          ...
          <div className="grid lg:grid-cols-12 gap-10">
            <div className="lg:col-span-7 space-y-8 bg-slate-900/40 border border-slate-800 rounded-[48px] p-8 shadow-2xl backdrop-blur-sm">
              <div className="space-y-6">
                ...
                <div className="rounded-[32px] overflow-hidden border-4 border-slate-950 shadow-2xl h-[400px]">
                  <MapContainer center={location ? [location.lat, location.lng] : [20.5937, 78.9629]} zoom={location ? 15 : 5} className="h-full w-full z-0">
            ...
            <div className="lg:col-span-5 bg-slate-900/40 border border-slate-800 rounded-[48px] p-8 md:p-10 shadow-2xl backdrop-blur-sm space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Formatted Address</label>
                  <textarea value={address} onChange={(e) => setAddress(e.target.value)} className="w-full mt-2 bg-slate-950 p-4 rounded-2xl border border-slate-800 outline-none font-bold text-sm text-white h-24" />
                </div>
              </div>
              ...
              >
                {isSubmitting ? <Loader2 className="animate-spin" size={20} /> : "Update Location"}
              </button>
            </div>
          </div>
        </div>
